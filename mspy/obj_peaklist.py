...
class peaklist:
    ...
    def deconvolute(self, massType=0):
        ...
        peaks = mod_peakpicking.deconvolute(peaklist=self, massType=massType)
    ...
    def consolidate(self, window, forceWindow=False):
        ...
        for current in self.peaks[1:]:
            ...
            if not forceWindow and previous.fwhm and current.fwhm:
                win = (previous.fwhm + current.fwhm) / 8.0
            ...
            if (previous.mz + win) > current.mz:
                ...
                # get m/z
                mz = (
                    previous.mz * previous.intensity + current.mz * current.intensity
                ) / intensity
                ...
                if previous.fwhm and current.fwhm:
                    fwhm = (
                        previous.fwhm * previous.intensity
                        + current.fwhm * current.intensity
                    ) / intensity
    ...
    def remthreshold(self, absThreshold=0.0, relThreshold=0.0, snThreshold=0.0):
        ...
        indexes = []
        for x, peak in enumerate(self.peaks):
            if peak.intensity < threshold or (
                peak.sn is not None and peak.sn < snThreshold
            ):
                indexes.append(x)
