...
def interpolate(p1, p2, x=None, y=None):
    ...
    if x is not None:
        return calculations.signal_interpolate_y(
            float(p1[0]), float(p1[1]), float(p2[0]), float(p2[1]), float(x)
        )

    # interpolate x point
    elif y is not None:
        return calculations.signal_interpolate_x(
            float(p1[0]), float(p1[1]), float(p2[0]), float(p2[1]), float(y)
        )
    ...
    if window is None:
        ...
        return numpy.array(
            [
                ...
                [signal[-1][0], noiseLevel, noiseWidth],
            ]
        )
